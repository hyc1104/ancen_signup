...
<h4 class="my">
    <small>
        <div><i class="fa fa-calculator" aria-hidden="true"></i> 活動日期：<{$block.action_date|substr:0:-3}></>
...
<a href="<{$xoops_url}>/modules/ancen_signup/index.php?op=ancen_signup_data_create&action_id=<{$block.id}>" class="btn btn-lg btn-info <{if !($block.enable && ($block.number + $block.candidate) > $block.signup|@count && $xoops_isuser && $block.end_date|strtotime >= $smarty.now)}>disabled<{/if}>">
