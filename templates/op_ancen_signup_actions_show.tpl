<h2 class="my">
    ...
    <{/if}>
    <{$title}>
<small><i class="fa fa-calculator" aria-hidden="true"></i> 活動日期：<{$action_date}></small>
</h2>
...
<table class="table" data-toggle="table" data-pagination="true" data-search="true" data-mobile-responsive="true">
    <thead>
        <tr>
            ...
            <{/foreach}>
            <th nowrap class="c" data-sortable="true">錄取</th>
            <th nowrap class="c" data-sortable="true">報名日期</th>
                        ...
                        <td>
                            <{if $signup_data.accept==='1'}>
                                ...
                                <{if $smarty.session.can_add && $uid == $now_uid}>
                                    <a href="<{$xoops_url}>/modules/ancen_signup/index.php?op=ancen_signup_data_accept&id=<{$signup_data.id}>&action_id=<{$id}>&accept=0" class="btn btn-sm btn-warning">改成未錄取</a>
                                <{/if}>
                            <{elseif $signup_data.accept==='0'}>
                                <div class="text-danger">未錄取</div>
                                <{if $smarty.session.can_add && $uid == $now_uid}>
                                    <a href="<{$xoops_url}>/modules/ancen_signup/index.php?op=ancen_signup_data_accept&id=<{$signup_data.id}>&action_id=<{$id}>&accept=1" class="btn btn-sm btn-success">改成錄取</a>
                                <{/if}>
                            <{else}>
                                <div class="text-muted">尚未設定</div>
                                <{if $smarty.session.can_add && $uid == $now_uid}>
                                    <a href="<{$xoops_url}>/modules/ancen_signup/index.php?op=ancen_signup_data_accept&id=<{$signup_data.id}>&action_id=<{$id}>&accept=0" class="btn btn-sm btn-warning">未錄取</a>
                                    <a href="<{$xoops_url}>/modules/ancen_signup/index.php?op=ancen_signup_data_accept&id=<{$signup_data.id}>&action_id=<{$id}>&accept=1" class="btn btn-sm btn-success">錄取</a>
                                <{/if}>
                            <{/if}>
...
<{if $smarty.session.can_add && $uid == $now_uid}>
    <div class="bar">
        <a href="javascript:del_action('<{$id}>')" class="btn btn-danger"><i class="fa fa-times" aria-hidden="true"></i> 刪除</a>
        <a href="<{$xoops_url}>/modules/ancen_signup/index.php?op=ancen_signup_actions_edit&id=<{$id}>" class="btn btn-warning"><i class="fa fa-pencil" aria-hidden="true"></i> 編輯</a>
    ...
    <form action="index.php" method="post" class="my-4" enctype="multipart/form-data">
        <div class="input-group">
            ...
            <div class="input-group-append input-group-btn">
                ...
                <a href="<{$xoops_url}>/modules/ancen_signup/csv.php?id=<{$id}>" class="btn btn-secondary"><i class="fa fa-text-o" aria-hidden="true"></i> 下載 CSV 匯入格式檔</a>
            </div>
        </div>
    ...
    <form action="index.php" method="post" class="my-4" enctype="multipart/form-data">
        <div class="input-group">
            ...
            <div class="input-group-append input-group-btn">
                ...
                <a href="<{$xoops_url}>/modules/ancen_signup/excel.php?id=<{$id}>" class="btn btn-secondary"><i class="fa fa-file-excel-o" aria-hidden="true"></i> 下載 Excel 匯入格式檔</a>
            </div>
        </div>
